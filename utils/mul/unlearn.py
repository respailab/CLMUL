...
from utils.mul.dataset import UnLearningData
import numpy as np
from utils import *
...
def UnlearnerLossOnlyBadTeacher(output, unlearn_teacher_logits, KL_temperature):
    u_teacher_out = F.softmax(unlearn_teacher_logits / KL_temperature, dim=1)
    student_out = F.log_softmax(output / KL_temperature, dim=1)
    return F.kl_div(student_out, u_teacher_out,reduction='batchmean')
...
def unlearning_step(model, unlearning_teacher, full_trained_teacher, inputs, labels, optimizer, device, KL_temperature):
