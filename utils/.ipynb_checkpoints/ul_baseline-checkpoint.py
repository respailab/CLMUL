...
def train(model: ContinualModel, dataset: ContinualDataset,
    ...
    # List of task indices to exclude
    tasks_to_exclude = [1, 3]  # Modify this list as needed

    # total 5 tasks
    for t in range(0, 5):
        if t in tasks_to_exclude:
            print(f'Skipping task {(t+1)}')
            continue
        ...
        for epoch in range(args.n_epochs):
            ...
            for i, data in enumerate(tqdm(train_loader)):
                ...
                # Learning step
                cl_loss = model.observe(inputs, labels, not_aug_inputs,
                                        ulabel=torch.zeros(inputs.shape[0]), unlearning_teacher=unlearning_teacher,
                                        full_trained_teacher=model, task_label=torch.tensor(t).repeat(inputs.shape[0]))
                loss = cl_loss
    ...
    print('Unlearning Process ended(retraining from scratch):')
