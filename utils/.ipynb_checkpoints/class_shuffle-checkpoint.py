...
# Define a custom class order
CUSTOM_CLASS_ORDER = [2, 0, 1, 5, 4, 3, 6, 8, 7, 9]

def remap_labels(labels, class_order):
    """
    Remaps labels to the custom class order.
    :param labels: the original labels
    :param class_order: the custom class order
    :return: remapped labels
    """
    return torch.tensor([class_order.index(label.item()) for label in labels])
    ...
    for t in range(0, 4):
        ...
        for epoch in range(args.n_epochs):
            ...
            for i, data in enumerate(tqdm(train_loader)):
                ...
                #print('Label:',labels)
                inputs, labels = inputs.to(model.device), remap_labels(labels, CUSTOM_CLASS_ORDER).to(model.device)
        ...
        for epoch in range(args.n_epochs):
            ...
            for i, data in enumerate(tqdm(train_loader)):
                ...
                for i in range(len(labels)):
                    if labels[i] == 1 or labels[i] == 5 or labels[i] == 6 or labels[i] == 8:
                    #if labels[i] == 5 or labels[i] == 6 or labels[i] == 7 or labels[i] == 8 or labels[i] == 9:
                        ulabels.append(1)
                    else:
                        ulabels.append(0)
