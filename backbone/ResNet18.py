...
class ResNet(nn.Module):
    ...
    def __init__(self, block: BasicBlock, num_blocks: List[int],
        ...
        self.inv_head = nn.Sequential(
            ...
            nn.BatchNorm1d(nf * 8 * block.expansion),
            nn.ReLU(inplace=True),
            nn.Linear(nf * 8 * block.expansion, 64)
        )
    ...
    def forward(self, x: torch.Tensor, return_features = False) :
        ...
        if return_features:
            #return self.linear(out), self.eq_head(out), F.normalize(self.inv_head(out), dim=1), F.normalize(out, dim=1)
            return self.linear(out),self.eq_head(out),F.normalize(self.inv_head(out), dim=1)
        else:
